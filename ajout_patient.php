...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['ajouter'])) {
        ...
        $conn->query("INSERT INTO patients (nom, prenom, age, email) VALUES ('$nom', '$prenom', $age, '$email')");
        header("Location: " . $_SERVER["PHP_SELF"]); // Empêche la double soumission
        exit();
    }
    if (isset($_POST['vider'])) {
        $conn->query("DELETE FROM patients");
        header("Location: " . $_SERVER["PHP_SELF"]); // Redirige après vidage
        exit();
    }
}
?>
...
<body>
    ...
    <!-- Formulaire pour vider la liste -->
    ...
    <table>
        ...
        <?php
        $result = $conn->query("SELECT nom, prenom, age, email FROM patients ORDER BY id DESC");
